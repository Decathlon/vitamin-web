...
program
  ...
  .option(
    '-o, --output [output]',
    'Specifies output file ("./sprite.svg" by default)',
  )
  .option(
    '-v, --viewbox [viewbox]',
    'Specifies viewBox attribute (parsed by default)',
  )
  .option(
    '-p, --prefix [prefix]',
    'Specifies prefix for id attribute (none by default)',
  )
...
const processFiles = (files) => {
  const processedFiles = files.filter(filterFile).map(processFile);
